...
int main(int argc, char** argv) {
    ...
    imageMeta textureImageMeta;
        
    filename = "images/marble.png";
    printf("load %s file...\n", filename);
    containerTextureData = stbi_load(filename, &textureImageMeta.x, &textureImageMeta.y, &textureImageMeta.colors, 0);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, (textureImageMeta.colors == 3 ? GL_RGB : GL_RGBA), textureImageMeta.x, textureImageMeta.y, 0, (textureImageMeta.colors == 3 ? GL_BGR : GL_ABGR_EXT), GL_UNSIGNED_BYTE, containerTextureData);
    printf("load %s width: %d, height: %d, colors: %d\n", filename, textureImageMeta.x, textureImageMeta.y, textureImageMeta.colors);
