...
void drawModel(GLuint shaderID, model *currentModel, u_int instances) {
    ...
    for(int i = 0; i < currentModel->numberOfMeshes; i++) {
        ...
        if(instances) {
            glDrawElementsInstanced(GL_TRIANGLES, currentModel->meshes[i]->numberOfIndexes, GL_UNSIGNED_INT, 0, instances);
        } else {
