...
int main(int argc, char** argv) {
    ...
    GLuint modelInstancingShaderId;
    modelInstancingShaderId = loadShader("shaders/model_instancing_shader.vs", "shaders/model_instancing_shader.fs");
    if(modelInstancingShaderId == -1) {
        return (EXIT_FAILURE);
    }
    ...
    glUseProgram(modelInstancingShaderId);
    GLuint modelInstancingModelID = glGetUniformLocation(modelInstancingShaderId, "model");
    GLuint modelInstancingViewID = glGetUniformLocation(modelInstancingShaderId, "view");
    GLuint modelInstancingProjectionID = glGetUniformLocation(modelInstancingShaderId, "projection");
    // GLuint cameraPositionID = glGetUniformLocation(modelShaderId, "cameraPos");
    // shaderTextureId = glGetUniformLocation(modelShaderId, "cubemap");
    // printf("shaderTextureId %d\n", shaderTextureId);
    if(modelInstancingModelID == -1 || modelInstancingViewID == -1 || modelInstancingProjectionID == -1) {
        printf("Failed to locate uniform variable location modelInstancingShaderId\n");
        return (EXIT_FAILURE);
    }
    ...
    #define cubeTotal 10
    // GLfloat cubeClonePositions[cubeTotal][cubeTotal][cubeTotal][3] = { };
    // for (i = 0; i < cubeTotal; i++) {
    //     for (int j = 0; j < cubeTotal; j++) {
    //         for (int k = 0; k < cubeTotal; k++) {
    //             cubeClonePositions[i][j][k][0] = -5.0f + j * 1.3f;
    //             cubeClonePositions[i][j][k][1] = 0.0f + i * 1.3f;
    //             cubeClonePositions[i][j][k][2] = -5.0f - k * 1.3f;
    //         }
    //     }
    // }
    ...
    for (i = 0; i < cubeTotal; i++) {
        for (int j = 0; j < cubeTotal; j++) {
            for (int k = 0; k < cubeTotal; k++) {
                ...
                sprintf(index_string, "%d", i * cubeTotal * cubeTotal + j * cubeTotal + k);
                strcpy(offset_index_string, "offsets[");
                strcat(offset_index_string, index_string);
                strcat(offset_index_string, "]");
                printf("[%d] %f %f %f\n", i * cubeTotal * cubeTotal + j * cubeTotal + k,
                offsets[i * cubeTotal * cubeTotal + j * cubeTotal + k][0],
                offsets[i * cubeTotal * cubeTotal + j * cubeTotal + k][1], 
                offsets[i * cubeTotal * cubeTotal + j * cubeTotal + k][2]);
                glUniform3fv(glGetUniformLocation(modelInstancingShaderId, offset_index_string), 1, (const GLfloat *)offsets[i * cubeTotal * cubeTotal + j * cubeTotal + k]);
            }
        }
    }
    ...
    while(glfwWindowShouldClose(window) == 0) {
        ...
        // drawModel(modelShaderId, boxModel);
        // drawModel(modelShaderId, cylinderModel);
        // drawModel(modelShaderId, skullModel);
        // drawModel(modelShaderId, sphereModel);
        drawModel(modelShaderId, nanosuitModel, NULL);
        // drawModel(modelShaderId, handgunModel);
        // drawModel(modelShaderId, skull3Model);
        ...
        // for (i = 0; i < cubeTotal; i++) {
            ...
            drawModel(modelInstancingShaderId, boxModel, cubeTotal * cubeTotal * cubeTotal);
            // drawModel(modelShaderId, handgunModel);
        // }
