...
int main(int argc, char** argv) {
    ...
    float pointLightAmbient[3] = { 0.1f, 0.1f, 0.1f };
    float pointLightDiffuse[3] = { 0.8f, 0.8f, 0.8f };
    ...
    float spotLightOuterCutOff = cos(deg2rad(17.5f));
    float spotLightAmbient[3] = { 0.1f, 0.1f, 0.1f };
    ...
    glUniform3fv(directionLightDirectionID, 1, &directionLightDirection[0]);
    glUniform3fv(directionLightAmbientID, 1, &directionLightAmbient[0]);
    glUniform3fv(directionLightDiffuseID, 1, &directionLightDiffuse[0]);
    glUniform3fv(directionLightSpecularID, 1, &directionLightSpecular[0]);
    for(i = 0; i < POINT_LIGHTS; i++) {
        glUniform3fv(pointLightAmbientID[i], 1, &pointLightAmbient[0]);
        glUniform3fv(pointLightDiffuseID[i], 1, &pointLightDiffuse[0]);
        glUniform3fv(pointLightSpecularID[i], 1, &pointLightSpecular[0]);
    ...
    float lastTimeOffPointLight = 10.0f;
    uint8_t offPointLight = 0;
    float offPointLightPosition[3] = {
        pointLightPositions[0][0],
        pointLightPositions[0][1],
        pointLightPositions[0][2],
    };
    ...
    while(glfwWindowShouldClose(window) == 0) {
        ...
        if(currentFrame - lastTimeOffPointLight > 1.5) {
            pointLightPositions[offPointLight][0] = offPointLightPosition[0];
            pointLightPositions[offPointLight][1] = offPointLightPosition[1];
            pointLightPositions[offPointLight][2] = offPointLightPosition[2];
            
            offPointLight++;
            if(offPointLight >= POINT_LIGHTS) {
                offPointLight = 0;
            }
            
            offPointLightPosition[0] = pointLightPositions[offPointLight][0];
            offPointLightPosition[1] = pointLightPositions[offPointLight][1];
            offPointLightPosition[2] = pointLightPositions[offPointLight][2];
            
            pointLightPositions[offPointLight][0] = -1000.0;
            pointLightPositions[offPointLight][1] = -1000.0;
            pointLightPositions[offPointLight][2] = -1000.0;
            
            lastTimeOffPointLight = currentFrame;
        }
