...
int main(int argc, char** argv) {
    ...
    static const GLfloat cubePosition[] = { -0.5f, -0.01f, -3.0f };
    ...
    GLuint shaderSingleColorId;
    shaderSingleColorId = loadShader("shader.vs", "shader_single_color.fs");
    if(shaderSingleColorId == -1) {
        return (EXIT_FAILURE);
    }
    ...
    GLuint shaderSingleColorModelID = glGetUniformLocation(shaderSingleColorId, "model");
    GLuint shaderSingleColorViewID = glGetUniformLocation(shaderSingleColorId, "view");
    GLuint shaderSingleColorProjectionID = glGetUniformLocation(shaderSingleColorId, "projection");
    ...
    // float scale[3] = { 2.1f, 2.1f, 2.1f };
    float scale[3] = { 1.1f, 1.1f, 1.1f };
    ...
    while(glfwWindowShouldClose(window) == 0) {
        ...
        // for(i = 0; i < CUBES * 3; i += 3) {
            ...
            matrix_identity((GLfloat *)modelMatrix);
            matrix_translate((GLfloat *)modelMatrix, (GLfloat *)&cubePosition);
            // float angel = 20.0f * i / 3;
            // matrix_rotate((GLfloat *)modelMatrix, angel, axis);
            glUniformMatrix4fv(modelID, 1, GL_TRUE, (const float *)modelMatrix);
        ...
        glUniformMatrix4fv(shaderSingleColorViewID, 1, GL_TRUE, (const float *)viewMatrix);
        glUniformMatrix4fv(shaderSingleColorProjectionID, 1, GL_TRUE, (const float *)projectionMatrix);
        ...
        // // for(i = 0; i < CUBES * 3; i += 3) {
            ...
            // float angel = 20.0f * i / 3;
            // matrix_rotate((GLfloat *)modelMatrix, angel, axis);
            matrix_scale((GLfloat *)modelMatrix, scale);
            glUniformMatrix4fv(shaderSingleColorModelID, 1, GL_TRUE, (const float *)modelMatrix);
