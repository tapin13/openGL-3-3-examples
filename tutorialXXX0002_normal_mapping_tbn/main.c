...
int main(int argc, char** argv) {
    ...
    // const vec3 pos1 = { 1.0f, 0.0f, 1.0 };
    // const vec3 pos2 = { -1.0f, 0.0f, 1.0 };
    // const vec3 pos3 = { -1.0f, 0.0f, -1.0 };
    // const vec3 pos4 = { 1.0f, 0.0f, -1.0 };

    // const vec3 pos1 = { 1.0f, 1.0f, 0.0 };
    // const vec3 pos2 = { -1.0f, -1.0f, 0.0 };
    // const vec3 pos3 = { 1.0f, -1.0f, 0.0 };
    // const vec3 pos4 = { -1.0f, 1.0f, 0.0 };
    ...
    const vec2 uv1 = { 0.0f, 1.0f };
    const vec2 uv2 = { 0.0f, 0.0f };
    const vec2 uv3 = { 1.0f, 0.0f };
    const vec2 uv4 = { 1.0f, 1.0f };

    const vec3 nm = { 0.0f, 0.0f, 1.0f };

    vec3 tangent1, tangent2;
    vec3 bitangent1, bitangent2;

    // --- triangle 1 ---
    vec3 edge1 = { pos2.x - pos1.x, pos2.y - pos1.y, pos2.z - pos1.z };
    vec3 edge2 = { pos3.x - pos1.x, pos3.y - pos1.y, pos3.z - pos1.z };

    vec2 deltaUV1 = { uv2.x - uv1.x, uv2.y - uv1.y };
    vec2 deltaUV2 = { uv3.x - uv1.x, uv3.y - uv1.y };
    ...
    // --- triangle 2 ---
    edge1 = (vec3) { pos3.x - pos1.x, pos3.y - pos1.y, pos3.z - pos1.z };
    edge2 = (vec3) { pos4.x - pos1.x, pos4.y - pos1.y, pos4.z - pos1.z };

    deltaUV1 = (vec2) { uv3.x - uv1.x, uv3.y - uv1.y };
    deltaUV2 = (vec2) { uv4.x - uv1.x, uv4.y - uv1.y };
    ...
    normalShaderId = loadShader("shaders/normal_tbn.vs", "shaders/normal_tbn.fs");
    if(normalShaderId == -1) {
        return (EXIT_FAILURE);
    }
    ...
    glVertexAttribPointer(
        ...
        sizeof(GLfloat) * 14, // stride - space between consecutive vertex attributes
    ...
    glVertexAttribPointer(
        3, //  position vertex attribute in the vertex shader with layout (location = 0)
        3, //  size of the vertex attribute. The vertex attribute is a vec3 so it is composed of 3 values.
        GL_FLOAT, // type of the data which is GL_FLOAT
        GL_FALSE, // we want the data to be normalized. But we don't want, data normalized. From 0 to 1.
        sizeof(GLfloat) * 14, // stride - space between consecutive vertex attributes
    ...
    glVertexAttribPointer(
        4, //  position vertex attribute in the vertex shader with layout (location = 0)
        3, //  size of the vertex attribute. The vertex attribute is a vec3 so it is composed of 3 values.
        GL_FLOAT, // type of the data which is GL_FLOAT
        GL_FALSE, // we want the data to be normalized. But we don't want, data normalized. From 0 to 1.
        sizeof(GLfloat) * 14, // stride - space between consecutive vertex attributes
    ...
    GLfloat lightPosition[] = { 0.0f, 0.5f, 0.0f };
    GLfloat lightColor[] = { 0.0f, 0.0f, 1.0f };
    float lightPositionStep = 1.0;
    float lightPositionMove = lightPositionStep;

    GLfloat axes[] = { 1.0f, 0.0f, 0.0f };
    float wallRotateAngel = -45;
    ...
    while(glfwWindowShouldClose(window) == 0) {
        ...
        matrix_identity((GLfloat *)modelMatrix);
        matrix_rotate((GLfloat *)modelMatrix, wallRotateAngel, axes);
