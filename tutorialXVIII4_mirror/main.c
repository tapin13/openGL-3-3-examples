...
int main(int argc, char** argv) {
    ...
    screenShaderId = loadShader("screen.vs", "screen.fs");
    if(screenShaderId == -1) {
        return (EXIT_FAILURE);
    }
    ...
    GLuint textureColoredBuffer;
    glGenTextures(1, &textureColoredBuffer);
    glBindTexture(GL_TEXTURE_2D, textureColoredBuffer);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, windowWidth, windowHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glBindTexture(GL_TEXTURE_2D, 0); // unbind
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textureColoredBuffer, 0);
    ...
    shaderTextureId = glGetUniformLocation(shaderTextureId, "screenTexture");
    // printf("shaderTextureId %d\n", shaderTextureId);
    glUniform1i(shaderTextureId, 0);
    ...
    while(glfwWindowShouldClose(window) == 0) {
        ...
        matrix_identity((GLfloat *)viewMatrix);
        cameraPositionFront[0] = cameraPosition[0] + cameraFront[0] * -1;
        cameraPositionFront[1] = cameraPosition[1] + cameraFront[1] * -1;
        cameraPositionFront[2] = cameraPosition[2] + cameraFront[2] * -1; 
    ...
    glBindVertexArray(0);
    glDeleteBuffers(1, &boxVertexBufferObjectID);
    glDeleteBuffers(1, &floorVBO);
    glDeleteBuffers(1, &grassVBO);
    glDeleteBuffers(1, &quadVBO);
    glDeleteBuffers(1, &mirrorVBO);
    glDeleteVertexArrays(1, &boxVertexArrayObjectID);
    glDeleteVertexArrays(1, &floorVAO);
    glDeleteVertexArrays(1, &grassVAO);
    glDeleteVertexArrays(1, &quadVAO);
    glDeleteVertexArrays(1, &mirrorVAO);    
    glDeleteTextures(1, &containerTextureID);
