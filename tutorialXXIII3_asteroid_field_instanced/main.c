...
int main(int argc, char** argv) {
    ...
    // };
    for(u_int i = 0; i < sizeof(textures_faces) / sizeof(textures_faces[0]); i++) {
        printf("load %s file...\n", textures_faces[i]);
    ...
    int instalceMatrixID = glGetAttribLocation(modelInstancedArrayShaderId, "instalceMatrix");
    // GLuint cameraPositionID = glGetUniformLocation(modelShaderId, "cameraPos");
    // shaderTextureId = glGetUniformLocation(modelShaderId, "cubemap");
    // printf("shaderTextureId %d\n", shaderTextureId);
    if(modelInstancingViewID == -1 || modelInstancingProjectionID == -1) {
    ...
    float planetTranslate[3] = { 0.0f, -3.0f, 0.0f };
    float planetModelScale[3] = { 3.0f, 3.0f, 3.0f };

    u_int asteroids = 1;
    if(argc > 0 && argv[1]) {
        asteroids = atoi(argv[1]);
    }
    
    float asteroidTempMatrix[4];
    float asteroidsMatrix[asteroids][4][4];
    memset(asteroidsMatrix, 0.0, asteroids * 4 * 4 * sizeof(float));

    float asteroidRadius = 50.0 + asteroids * 0.003;
    float asteroidOffset = 5.0f + asteroids * 0.005;

    srand(glfwGetTime());
    for(u_int i = 0; i < asteroids; i++) {
        matrix_identity((GLfloat *)asteroidsMatrix[i]);

        float angel = (float)i / (float)asteroids * 360.0f;
        float displacement = (rand() % (int)(2 * asteroidOffset * 100)) / 100.0f - asteroidOffset;
        float x = sin(angel) * asteroidRadius + displacement;
        asteroidTempMatrix[0] = x;
        displacement = (rand() % (int)(2 * asteroidOffset * 100)) / 100.0f - asteroidOffset;
        float y = displacement * 0.4f;
        asteroidTempMatrix[1] = y;
        displacement = (rand() % (int)(2 * asteroidOffset * 100)) / 100.0f - asteroidOffset;
        float z = cos(angel) * asteroidRadius + displacement;
        asteroidTempMatrix[2] = z;
        matrix_translate((GLfloat *)asteroidsMatrix[i], (GLfloat *)asteroidTempMatrix);

        float scale = (rand() % 20) / 100.0f + 0.05;
        asteroidTempMatrix[0] = scale;
        asteroidTempMatrix[1] = scale;
        asteroidTempMatrix[2] = scale;
        matrix_scale((GLfloat *)asteroidsMatrix[i], (GLfloat *)asteroidTempMatrix);

        float rotateAngle = (rand() % 360);
        asteroidTempMatrix[0] = 0.4f;
        asteroidTempMatrix[1] = 0.6f;
        asteroidTempMatrix[2] = 0.8f;
        matrix_rotate((GLfloat *)asteroidsMatrix[i], rotateAngle, (GLfloat *)asteroidTempMatrix);

        // flip matrix for OpenGL (read by columns not by rows)
        matrix_to_column_based((GLfloat *)asteroidsMatrix[i]);
    }

    u_int bufferID;
    glGenBuffers(1, &bufferID);
    glBindBuffer(GL_ARRAY_BUFFER, bufferID);
    glBufferData(GL_ARRAY_BUFFER, sizeof(asteroidsMatrix), asteroidsMatrix, GL_STATIC_DRAW);
    for(u_int i = 0; i <  rockModel->numberOfMeshes; i++) {
        glBindVertexArray(rockModel->meshes[i]->VAO);

        for(u_int i = 0; i < 4; i++) {
            glEnableVertexAttribArray(instalceMatrixID + i);
            glVertexAttribPointer(instalceMatrixID + i, 4, GL_FLOAT, GL_FALSE, 4 * 4 * sizeof(float), (void *)(i * 4 * sizeof(float)));
            glVertexAttribDivisor(instalceMatrixID + i, 1);
        }
    ...
    while(glfwWindowShouldClose(window) == 0) {
        ...
        if(currentTime - lastTime >= 1.0){ // If last prinf() was more than 1 sec ago
            printf("objects: %d - %d ms/frame - ", asteroids, nbFrames);
            nbFrames = 0;
            lastTime = glfwGetTime();

            // printf("currentFrame: %f\n", currentFrame);
            printf("deltaTime: %f ms\n", deltaTime * 1000);
        }        
        ...
        matrix_identity((GLfloat *)modelMatrix);
        matrix_translate((GLfloat *)modelMatrix, (GLfloat *)planetTranslate);
        matrix_scale((GLfloat *)modelMatrix, planetModelScale);
        glUniformMatrix4fv(modelModelID, 1, GL_TRUE, (const float *)modelMatrix);
        modelDraw(modelShaderId, planetModel, NULL);
        ...
        modelDraw(modelInstancedArrayShaderId, rockModel, asteroids);
