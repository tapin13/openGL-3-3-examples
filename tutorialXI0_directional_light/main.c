...
int main(int argc, char** argv) {
    ...
    bmp_load("emission_map.bmp", &containerTextureData, &containerTextureSize[0], &containerTextureSize[1]);
    
    GLuint containerEmissionTextureID;
    glGenTextures(1, &containerEmissionTextureID);
    glBindTexture(GL_TEXTURE_2D, containerEmissionTextureID);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);    
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, containerTextureSize[0], containerTextureSize[1], 0, GL_BGR, GL_UNSIGNED_BYTE, containerTextureData);
    ...
    GLuint materialEmissionID = glGetUniformLocation(programId, "material.emission");
    ...
    glBindTexture(GL_TEXTURE_2D, containerEmissionTextureID);
    glUniform1i(materialEmissionID, 2);
    ...
    while(glfwWindowShouldClose(window) == 0) {
        ...
        for(i = 0; i < 10 * 3; i += 3) {
            matrix_identity((GLfloat *)modelMatrix);
            matrix_translate(&modelMatrix, &cubePositions[i]);
            float angel = 20.0f * i / 3;
            matrix_rotate(&modelMatrix, angel, axis);
