...
void matrix_translate(float translate_x, float translate_y, float translate_z, GLfloat *matrix);
...
//    translate_y = -0.5f;
    ...
    GLfloat axes[3] = { 1.0f, 0.0f, 0.0f };
    matrix_rotate(*modelMatrix, -55.0f, axes);
    matrix_print(*modelMatrix);

    matrix_translate(0.0f, 0.0f, -0.5f, (GLfloat *)viewMatrix);
//    matrix_print(*viewMatrix);
    ...
    matrix_perspective(45.0f, (float)windowWidth / (float)windowHeight, 0.1f, 100.0f, (GLfloat *)projectionMatrix);
    matrix_print(*projectionMatrix);
...
void matrix_translate(float translate_x, float translate_y, float translate_z, GLfloat *matrix) {
    matrix[0 * 4 + 3] = translate_x;
    matrix[1 * 4 + 3] = translate_y;
    matrix[2 * 4 + 3] = translate_z;
}
...
void matrix_perspective(float angle, float ratio, float near, float far, GLfloat *matrix) {
    ...
    matrix[2 * 4 + 3] = -2.0f * far * near / (far - near);
    matrix[3 * 4 + 2] = -2.0f;
}
