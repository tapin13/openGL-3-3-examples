...
void matrix_translate(float translate_x, float translate_y, GLfloat *transformMatrix);
...
int main(int argc, char** argv) {
    ...
    while(glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS && glfwWindowShouldClose(window) == 0) {
        ...
        matrix_translate(translate_x, translate_y, (GLfloat *)transformMatrix);
        ...
        matrix_scale((float)scale, (GLfloat *)transformMatrix);
        glUniformMatrix4fv(transformID, 1, GL_TRUE, (const float *)transformMatrix);
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
...
void matrix_translate(float translate_x, float translate_y, GLfloat *transformMatrix) {
    transformMatrix[3] = translate_x;
    transformMatrix[7] = translate_y;
}
