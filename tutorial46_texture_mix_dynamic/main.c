...
int main(int argc, char** argv) {
    ...
    //float dirTextureMixPercent = 0.01f;    
    while(glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS && glfwWindowShouldClose(window) == 0) {
        ...
        /*
         * // change mix percent with time
        textureMixPercent += dirTextureMixPercent;
        if(textureMixPercent > 1.0) {
            dirTextureMixPercent *= -1.0f;
        }
        if(textureMixPercent < 0.0) {
            dirTextureMixPercent *= -1.0f;
        }
        */
    }
